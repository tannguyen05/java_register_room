		...
		if (session.getAttribute("userid") != null) {
			try {
				...
				PreparedStatement ps1 = conn.prepareStatement(
						"select register.id, ca, thu, mon, lop, buoithaythe, fullname, register.user_id from register inner join user on register.user_id = user.id where room_id="
								+ room);
				ResultSet rs1 = ps1.executeQuery();
				Register[][] list = new Register[3][7];
				PreparedStatement ps3 = conn.prepareStatement("select * from pratice_room where id="+room);
